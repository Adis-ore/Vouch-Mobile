    ...
    <View style={[styles.row, { borderBottomColor: colors.border }]}>
      ...
      <View style={styles.info}>
        <View style={styles.nameRow}>
          <Text style={[styles.name, { color: colors.textPrimary }]}>{member.full_name}</Text>
          {member.role === 'creator' && (
        ...
        <View style={styles.metaRow}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3 }}>
            ...
            <Text style={[styles.streak, { color: colors.textSecondary }]}>{member.current_streak}d streak</Text>
          </View>
          <View style={[styles.rateBar, { backgroundColor: colors.surfaceAlt }]}>
...
const styles = StyleSheet.create({
  ...
  creatorBadge: { backgroundColor: 'rgba(232,168,56,0.12)', borderRadius: 4, paddingHorizontal: 6, paddingVertical: 2, borderWidth: 1, borderColor: 'rgba(232,168,56,0.25)' },
  creatorText: { fontFamily: fonts.bodyMedium, fontSize: 10 },
  metaRow: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  streak: { fontFamily: fonts.body, fontSize: 12 },
  rateBar: { flex: 1, height: 4, borderRadius: 2, overflow: 'hidden' },
